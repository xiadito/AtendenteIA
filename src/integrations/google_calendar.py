...
def build_authorization_url(state: str) -> tuple[str, str]:
    ...
    Returns:
        tuple[str, str]: (authorization_url, code_verifier). The verifier is the
        PKCE secret generated by authorization_url(); the caller must persist it
        and hand it back to exchange_code_for_tokens() on the callback, since
        that runs on a different request with a different Flow instance.
    """
...
def exchange_code_for_tokens(code: str, code_verifier: str) -> tuple[str, str, str]:
    ...
    Args:
        ...
        code_verifier (str): PKCE verifier returned by build_authorization_url()
            on the /connect request. Google rejects the exchange without it
